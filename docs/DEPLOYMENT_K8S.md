...
local Bronze
  -> HDFS Bronze
  -> Spark on YARN Silver (taiwan + northwest_pacific)
...
目前每個 executor 需要 `4g + 1g overhead`、2 cores，共 3 個。若超過叢集上限，
先降低 ConfigMap 的 executor memory、overhead、cores 或 instances。不能靠
增加 Kubernetes Pod memory 解決 YARN 的限制。
...
`AOI_IDS=taiwan,northwest_pacific` 會依序建立：

```text
hdfs:///elt/ocean/silver/taiwan/nasa_daily_grid/
hdfs:///elt/ocean/silver/taiwan/gfw_daily_grid/
hdfs:///elt/ocean/silver/northwest_pacific/nasa_daily_grid/
hdfs:///elt/ocean/silver/northwest_pacific/gfw_daily_grid/
```
...
```text
/opt/zfs/project/data/serving/releases/2024_12/
/opt/zfs/project/data/serving/current -> releases/2024_12
```
...
curl 'http://<任一可達K8S_NODE_IP>:30800/api/v1/gold/daily-grid?date=2024-12-12&aoi=taiwan&product=COMBINED&metric=potential_fishing_score&resolution=4'
```
